...
public class Trie {
   ...
   public TrieNode addWord(String word, int p) {
      return root.addWord(word.toLowerCase(), p);
   }
   ...
   public String[] guessWords(String prefix, int p) {
      TrieNode node = root;
      ...
      List<String> guessNode = new ArrayList<>();         //Store 3 node that contains 3 guesses words
      ...
      if(p == 0){
        while(guessNode.size() < 3 && i < keys.size()) {
          ...
          if(n.isGuess == false && n.postion.contains(p)) {
            guessNode.add(n.toString());
            n.isGuess = true;
            guessList.add(n);
          }
          i++;
      ...
      return guessNode.toArray(new String[3]);
   }
