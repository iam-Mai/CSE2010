...
public class TrieNode {
   ...
   public Map<Integer, TrieNode> childrenT = new TreeMap<>();
   ...
   public void mapT() {
      TrieNode p = this;
      while(p.parent != null) {
          List<Integer> keys = new ArrayList(p.parent.childrenT.keySet());
          Comparator cmp = Collections.reverseOrder();
          Collections.sort(keys, cmp);
          if(keys.size() < 8) {
              p.parent.childrenT.put(this.frequence, this);
          }
          else {
              p.parent.childrenT.remove(keys.get(keys.size()-1));
              p.parent.childrenT.put(this.frequence, this);
          }
          p = p.parent;
      }
